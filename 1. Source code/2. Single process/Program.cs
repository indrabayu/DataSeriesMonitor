...
namespace TestingForUva
{
    class Program
    {
        ...
        static void gc()
        {
            ...
            //bool every_10_iterations = (successfulIterations % 10) == 0;
            //GC.Collect(2, GCCollectionMode.Forced, every_10_iterations, every_10_iterations);
        ...
        static void BeginSimulation()
        {
            ...
            SemaphoreSlim sema_stream0_with_stream1 = new SemaphoreSlim(0);
            SemaphoreSlim sema_stream0_with_stream2 = new SemaphoreSlim(0);

            var GENESIS = Stopwatch.StartNew();

            Func<TimeSpan, KeyValuePair<TimeSpan, int>> GetInput = (TimeSpan duration) =>
            {
                Thread.Sleep(duration);
                int inputValue = rnd.Next(0, 9999);
                return new KeyValuePair<TimeSpan, int>(GENESIS.Elapsed, inputValue);
            };
            ...
            var task0 = Task.Run(() =>
            {
                while (true)
                {
                    ...
                    var input = GetInput(sleepDuration);

                    concurrentQueueOfBridgeWith_1.Enqueue(input);
                    concurrentQueueOfBridgeWith_2.Enqueue(input);

                    sema_stream0_with_stream1.Release();
                    sema_stream0_with_stream2.Release();
            ...
            var task1 = Task.Run(() =>
            {
                do
                {
                    sema_stream0_with_stream1.Wait();
                    ...
                    Thread oneSecondDurationThread = new Thread(() =>
                    {
                        ...
                        try
                        {
                            ...
                            if (ProcessOnlyGen2 == true)
                            {
                                ...
                                do
                                {
                                    sema_stream0_with_stream2.Wait();
                            ...
                            if (doneProcessingGen2) //something needs to be printed out regardless whether this cycle was done or not
                            {
                                if (DEBUG && Print_Task_2)
                                {
                                    Console.WriteLine("[avg {0}] [qti {1}] [old {2}s] [you {3}s] [lat {4}s] [jit {5}s] [{6} MB] [~ {7}ms] [{8} i/s] [gc {9}ms] [{10}]{11}",
                                            ...
                                            /*  6 */ (Process.GetCurrentProcess().WorkingSet64 / 1048576 /* convert to Mega Byte */).ToString("000").Beautify(),
